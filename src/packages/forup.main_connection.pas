...
type
  ...
  private
    ...
    public
      ...
      procedure LoadResouce(aLoadPG, aLoadMongo : boolean);
...
begin
  {$IFDEF MSWINDOWS}
    if aLoadPG then
      begin
        pgDrvRes := TResourceStream.Create(HInstance, 'PGDRVW64', RT_RCDATA);
        zipHandle := TZipFile.Create;
        zipHandle.Open(pgDrvRes, zmRead);
